...
RUN apt-get update && apt-get install -y curl && curl -o- https://raw.githubusercontent.com/creationix/nvm/v0.33.9/install.sh | bash && export NVM_DIR="$HOME/.nvm" [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"
RUN cd / && pwd && ls
